    ...
    for line in file:
        ...
        if line[0] is not '#':
            ...
            with conn.cursor() as cursor:
                ...
                sql = "replace into overbejt.gene(seqname, g_start, g_end, score, frame, a_id, f_id, s_id, release_v) "
                sql += "values(%s, %s, %s, %s, %s, "
                sql += "(select attr_id from overbejt.attr where data=%s ), "
                sql += "(select feature_id from overbejt.features where feature=%s), "
                sql += "(select source_id from overbejt.source where source_name=%s), %s)"
                cursor.execute(sql, (arr[0], arr[3], arr[4], score, frame, arr[8], arr[2], arr[1], '82'))
            conn.commit()
finally:
    conn.close()
